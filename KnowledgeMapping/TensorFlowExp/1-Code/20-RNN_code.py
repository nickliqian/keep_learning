...
x = tf.placeholder(tf.int32, [batch_size, num_steps])  # [5, 10]
y = tf.placeholder(tf.int32, [batch_size, num_steps])  # [5, 10]
...
with tf.variable_scope('rnn_cell'):
    ...
    b = tf.get_variable('b', [state_size], initializer=tf.constant_initializer(0.0))  # [10]
...
losses = [tf.nn.sparse_softmax_cross_entropy_with_logits(labels=label, logits=logit) for label, logit in
          zip(y_as_lists, predictions)]
