...
function generateEncryptPassword(length, encryptPassChars) {
    var randomstring = '';
...
function pbkdf2(passPhrase, salt, data, iv){
...
def gen_key():
    pwd = execjs.compile(a)
    encryptPassChars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz*&-%/!?*+=()"
    data = "1qaz@WSX"
    passPhrase = pwd.call("generateEncryptPassword", 32, encryptPassChars)
    iv = pwd.call("random_str")
    salt = pwd.call("random_str")

    print(encryptPassChars)
    print(passPhrase)
    print(iv)
    print(salt)

    aesKey, encryptedMessage = pwd.call("pbkdf2", passPhrase, salt, data, iv)
    print("aesKey -> ", aesKey)
    ...
    EwIDAQAB
    -----END PUBLIC KEY-----"""
    # pbk = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCp0wHYbg/NOPO3nzMD3dndwS0MccuMeXCHgVlGOoYyFwLdS24Im2e7YyhB0wrUsyYf0/nhzCzBK8ZC9eCWqd0aHbdgOQT6CuFQBMjbyGYvlVYU2ZP7kG9Ft6YV6oc9ambuO7nPZh+bvXH0zDKfi02prknrScAKC0XhadTHT3Al0QIDAQAB"
    rsakey = RSA.importKey(pbk)
    cipher = PKCS1_OAEP.new(rsakey)
    print(cipher)
    cipher_text = base64.b64encode(cipher.encrypt(aesKey.encode()))
    print(cipher_text)
    encryptedKey = cipher_text.decode("utf-8")
    print("encryptedKey -> ", encryptedKey)
    encrypted = encryptedKey + ":::" + encryptedMessage
    print("encrypted -> ", encrypted)
    return encrypted

    # import rsa
    # rsakey = rsa.PublicKey.load_pkcs1(pbk.encode())
    # crypto = rsa.encrypt(aesKey.encode(), rsakey)
    # print(crypto)
