    ...
    def insert_sql(self, items, row):
        for item in items:
            sql = "INSERT INTO {} (area,industry,source_url,company_name,company_url) VALUE ('{}','{}','{}','{}','{}')"\
                .format(self.mysql_table, row[0], row[1], row[2], item["name"], item["href"])
            self.mysql_cursor.execute(sql)
            print("Write data <{}> <{}> <{}> <{}> <{}>".format(row[0], row[1], row[2], item["name"], item["href"]))
            ...
            try:
                ...
                if response.status_code == 200:
                    if (text.startswith("采集大神饶命")) or ("too many request" in text) or "":
                        time.sleep(1)
                        print("请求太频繁，将放回redis队列重新请求 {}".format(target_url))
    ...
    def parse_page(self, response):
        ...
        if results:
            ...
            if flag:
                ...
                print("出现了一点异常")
                self.add_log("出现了一点异常 <{}>".format(response.url))
        ...
        while not self.stop_flag:
            ...
            try:
                ...
                if isinstance(e, RequestError):
                    ...
                    print("手动中断，任务写回redis")
                else:
                    self.add_log("{} {}".format(str(row), e))
